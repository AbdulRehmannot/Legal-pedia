...
const AgentChatbot = ({ assitantKey, conversation, setConversation, id }) => {
  ...
  const { province, lawcategory } = useParams();

  const API_KEY = process.env.REACT_APP_API_KEY;
  const [ASSISTANT_ID, setAssistant_id] = useState(assitantKey);

  useEffect(() => {
    if (conversation?.length !== 0) {
      sessionStorage.setItem(`conversation${id}`, JSON.stringify(conversation));
    }
  }, [conversation]);

  useEffect(() => {
    setAssistant_id(assitantKey);
    console.log("assitantKey ", assitantKey);
  }, [assitantKey]);
  ...
  const initializeChat = async () => {
    try {
      const storedThreadId = sessionStorage.getItem(`${id}threadId`);
      if (storedThreadId) {
        ...
        setThreadId(thread.id);
        sessionStorage.setItem(`${id}threadId`, thread.id);
      }
    } catch (error) {
  ...
  const getStoredConversation = () => {
    setPageLoading(true);
    return sessionStorage.getItem(`conversation${id}`);
  };
